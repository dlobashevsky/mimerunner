...
echo -e "const struct mime_hash_t *mime_enc(register const char *str, register unsigned int len);\n\n" >$DST.h

cat $tmp | tr [a-z] [A-Z] | sed -e 's/-/__/g' -e 's/\./___/g' | nl -ba -v0 -s, -w1 | \
 sed -e 's/^\([0-9][0-9]*\),\(.*\)$/#define MIME_ENC_\2\t\1/' >>$DST.h

echo -e "\n#define MIME_ENC_COUNT\t" $count >>$DST.h
